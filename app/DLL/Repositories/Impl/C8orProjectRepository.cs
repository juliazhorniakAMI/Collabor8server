...
namespace app.DLL.Repositories.Impl
{
    public class C8orProjectRepository : GenericKeyRepository<int, C8orProject, DataContext>, IC8orProjectRepository
    {
        ...
        public async Task<ServiceResponse<bool>> AddC8orProject(C8orProjectBrowseDTO cp)
        {

            C8orProject r = _mapper.Map<C8orProjectBrowseDTO, C8orProject>(cp);
            return await Add(r);
        }
        public async Task<bool> UpdateC8orPjt(C8orProjectDashboardDTO cp)
        {
            var existingCP = Context.C8orsProjects.First(x => x.UserId == cp.UserId && x.SphereId == cp.SphereId && x.ProjectId == cp.ProjectId);
            existingCP.ProposedURL = cp.ProposedURL;
            existingCP.AgreementURL = cp.AgreementURL;
            existingCP.status = cp.status;
